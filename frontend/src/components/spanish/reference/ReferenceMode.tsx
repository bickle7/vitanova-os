...
import { Search, ArrowLeftRight, Plus, Compass } from 'lucide-react'
...
const CATEGORY_FILTERS: { id: Category | 'all' | 'favourites'; label: string; emoji: string }[] = [
...
function DiscoveryModal({ discoveryWords, onAdd, onClose }: DiscoveryModalProps) {
  ...
  const catFilters = CATEGORY_FILTERS.filter(c => c.id !== 'all' && c.id !== 'favourites') as { id: Category; label: string; emoji: string }[]
              ...
              {filtered.map(word => (
                ...
                >
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-accent-warm font-medium truncate">{word.spanish}</p>
                    {word.pronunciation && (
                  ...
                  <button
                    onClick={() => onAdd(word)}
                    className="ml-3 flex-shrink-0 w-8 h-8 rounded-full bg-accent/10 border border-accent/20 flex items-center justify-center text-accent press-active hover:bg-accent hover:text-white transition-all duration-200"
                  >
                    <Plus size={16} />
                  </button>
                </div>
...
export default function ReferenceMode({
  ...
  const [activeCategory, setActiveCategory] = useState<Category | 'all' | 'favourites'>('all')
  ...
  const filteredWords = useMemo(() => {
    return words.filter(word => {
      if (activeCategory === 'favourites' && !word.is_favourite) return false
      if (activeCategory !== 'all' && activeCategory !== 'favourites' && word.category !== activeCategory) return false
  ...
  }, [words, activeCategory, search, direction])
  ...
  const handleAddDiscoveryWord = (word: Word) => {
    addSeedWord(word)
  }
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="px-4 pt-1 pb-3 flex gap-2 flex-shrink-0">
        ...
        {filteredWords.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <h3 className="text-text-primary font-semibold mb-1">
              {search ? 'No results found' : activeCategory === 'favourites' ? 'No favourites yet' : 'No words yet'}
            </h3>
            <p className="text-text-muted text-sm max-w-[200px]">
              {search
                ...
                : 'Add words using the + button or browse the word bank'}
            </p>
            {!search && activeCategory !== 'favourites' && (
              <button
                onClick={() => setShowDiscovery(true)}
                className="mt-4 flex items-center gap-2 px-4 py-2.5 bg-accent/10 border border-accent/20 rounded-xl text-accent text-sm font-medium press-active hover:bg-accent hover:text-white transition-all duration-200"
              >
                <Compass size={15} />
                Browse word bank
              </button>
